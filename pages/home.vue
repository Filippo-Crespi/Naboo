<script lang="ts" setup>
import type { Response, FormData } from "~/types";

definePageMeta({
  layout: "header",
});
const toast = useToast();
const isDialogVisible = ref(false);
const router = useRouter();

onMounted(() => {
  useCookie("user").value = null;
  useCookie("token").value = null;
});

const modulo = ref<FormData>({
  Titolo: "",
  Descrizione: "",
});

const code = ref("");
const loading = ref(false);
const search = async () => {
  loading.value = true;
  code.value = code.value.replace(/\s/g, "");
  if (code.value.length < 5) {
    toast.add({
      severity: "warn",
      summary: "Attenzione",
      detail: "Il codice deve essere lungo almeno 5 caratteri",
      life: 2500,
    });
    loading.value = false;
    return;
  }
  try {
    const res: Response = await $fetch(
      `https://andrellaveloise.it/forms/users?Codice=${code.value}&data=Titolo+Descrizione`,
      {
        method: "GET",
        onResponseError({ response }) {
          throw new Error(response._data.message);
        },
      }
    );
    modulo.value.Titolo = res.data.Titolo;
    modulo.value.Descrizione = res.data.Descrizione;
    isDialogVisible.value = true;
    // redirect al modulo
  } catch (error) {
    toast.add({
      severity: "error",
      summary: "Errore",
      detail: (error as Error).message,
      life: 2500,
    });
  } finally {
    loading.value = false;
  }
};
</script>

<template>
  <Toast />
  <Dialog class="w-5/6 md:w-[unset]" modal v-model:visible="isDialogVisible" pt:mask:class="backdrop-blur-sm"
    header="Modulo trovato">
    <div>
      <p class="text-lg">
        Il modulo <strong>{{ modulo.Titolo }}</strong> è stato trovato con successo.
      </p>
      <p class="mt-2">
        Descrizione: <em>{{ modulo.Descrizione }}</em>
      </p>
      <div class="mt-4 flex justify-end">
        <Button label="Compila il modulo" icon="pi pi-pencil" class="p-button-success"
          @click="router.push('/compile/' + code)" />
      </div>
    </div>
  </Dialog>
  <div
    class="h-[100dvh] flex flex-col-reverse sm:flex-row bg-white sm:bg-[url('/imgs/home/background.png')] bg-cover bg-center">
    <div class="h-4/9 sm:w-1/2 sm:h-auto flex flex-col items-center justify-center">
      <Card class="px-2 sm:p-8 h-full sm:h-auto w-full sm:w-auto !rounded-none sm:!rounded justify-start">
        <template #title>
          <span class="sm:text-4xl text-2xl font-bold">Crea un modulo</span></template>
        <template #subtitle> <span>Account richiesto</span></template>
        <template #content>
          <div class="flex flex-col sm:flex-row gap-2">
            <Button as="router-link" to="/auth/login" label="Accedi" icon="pi pi-sign-in" />
            <Button label="Crea un account" icon="pi pi-user-plus" as="router-link" to="/auth/register" />
          </div>
        </template>
      </Card>
    </div>
    <div class="h-4/9 sm:w-1/2 sm:h-auto flex flex-col items-center justify-center">
      <Card class="px-2 sm:p-8 h-full sm:h-auto w-full sm:w-auto !rounded-none sm:!rounded justify-center">
        <template #title>
          <span class="sm:text-4xl text-2xl font-bold">Compila un modulo</span></template>
        <template #subtitle> <span>Codice richiesto</span></template>
        <template #content>
          <div class="flex flex-col gap-2">
            <InputText v-model="code" placeholder="Inserisci il codice" /><Button icon="pi pi-search" @click="search()"
              label="Cerca" :loading="loading" />
          </div>
        </template>
      </Card>
    </div>
  </div>
</template>